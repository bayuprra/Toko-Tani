                                    ...
                                    @foreach ($data as $kategori)
                                        <tr>
                                            <div class="row">
                                                ...
                                                <td class="col-6 deskripsi">
                                                    {{ $kategori->deskripsi != ' ' ? $kategori->deskripsi : 'Belum Ada Deskripsi' }}
                                                </td>
                                                <td class="d-flex flex-row" style="gap: 10px">
                                                    <button type="button" class="btn btn-warning btn-xs" style="width: 50%"
                                                        data-toggle="modal"
                                                        data-target="#modal-update-{{ $kategori->id }}"><i
                                                            class="fas fa-edit"></i></button>
                                                    <button type="button" class="btn  btn-danger btn-xs"
                                                        style="margin-top: 0;width: 50%" id="deleteButton"
                                                        onclick="hapusKategori({{ $kategori->id }});"><i
                                                            class="fas fa-solid fa-trash"></i></button>
                                                    <form action="{{ route('deleteKategori', ['id' => $kategori->id]) }}"
                                                        method="post" id="formHapusKategori{{ $kategori->id }}">
                                                        @method('delete')
                                                        @csrf
                                                    </form>
                                                </td>
